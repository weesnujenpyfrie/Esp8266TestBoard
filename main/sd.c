...
static int s_bitRate;							// 転送ビットレート[Hz]
...
//----- プロトタイプ宣言 -----
// FatFs要求関数
static DSTATUS Initialize(unsigned char pdrv);
static DSTATUS GetStatus(unsigned char pdrv);
static DRESULT ReadBlock(unsigned char pdrv, unsigned char* buff, uint32_t sector, unsigned count);
static DRESULT WriteBlock(unsigned char pdrv, const unsigned char* buff, uint32_t sector, unsigned count);
static DRESULT ControlIo(unsigned char pdrv, unsigned char cmd, void* buff);
...
static inline void SetRxMode(void);
static inline void SetTxMode(void);
static inline void StartCommunication(void);												// 通信開始
static void StopCommunication(void);														// 通信停止
...
//----------------------------------------------------------------------
DSTATUS Initialize(unsigned char pdrv)
{
	...
	//----- SPI設定 -----
	s_bitRate = 400000L;
	set_SetPin(PinSetting_SdMount, &s_bitRate);
	...
	case Card_SdVer1:
	case Card_MmcVer3:
		if(SendCom(16, 0x00000200, NULL, 1) != r1NoError)
		{
			s_cardType = Card_Unknown;
		}
		break;
	...
	if(s_cardType != Card_Unknown)
	{
		...
		else if(csdVer == 1)
		{
			...
			else
			{
				...
				s_allocationUnitSize = GetRegValue(info.u8, 47, 44);	// AU_SIZE [47:44]
				s_allocationUnitSize = auSizeTable[s_allocationUnitSize];
			}
		}
		else
		{
	...
	switch(s_cardType)
	{
	case Card_SdVer2Block:	printf("SD v2 block\n");	break;
	case Card_SdVer2Byte:	printf("SD v2 byte\n");		break;
	case Card_SdVer1:		printf("SD v1\n");			break;
	case Card_MmcVer3:		printf("MMC v3\n");			break;
	case Card_Unknown:
	default:				printf("Unknown\n");		break;
	}
	if(s_cardType != Card_Unknown)
	{
		printf("card size=%u erase unit=%u\n", s_cardSize, s_allocationUnitSize);
	}
...
//! @return	状態 次の値をORでつなげた値 STA_NOINIT, STA_NODISK, STA_PROTECT
//----------------------------------------------------------------------
DSTATUS GetStatus(unsigned char pdrv)
{
...
//----------------------------------------------------------------------
DRESULT ReadBlock(unsigned char pdrv, unsigned char* buff, uint32_t sector, unsigned count)
{
	ESP_LOGI(TAG, "called ReadBlock | dr=%02x sect=%u, cnt=%u", pdrv, sector, count);
...
//! @return	RES_OK=成功 RES_ERROR=R/Wエラー RES_WRPRT=書込禁止 RES_NOTRDY=未準備 RES_PARERR=パラメータ無効
//----------------------------------------------------------------------
DRESULT WriteBlock(unsigned char pdrv, const unsigned char* buff, uint32_t sector, unsigned count)
{
	ESP_LOGI(TAG, "called WriteBlock | dr=%02x sect=%u, cnt=%u", pdrv, sector, count);
...
//! @return	RES_OK=成功 RES_ERROR=R/Wエラー RES_WRPRT=書込禁止 RES_NOTRDY=未準備 RES_PARERR=パラメータ無効
//----------------------------------------------------------------------
DRESULT ControlIo(unsigned char pdrv, unsigned char cmd, void* buff)
{
	if((s_cardStatus & STA_NOINIT) != 0)
	{
		return RES_NOTRDY;
	...
	case CTRL_TRIM:
		...
		// 実装するならSDカードコマンド CMD32 消去開始ブロック, CMD33 消去終了ブロック, CMD38 ブロック消去 を使用して削除する
		break;
...
{
	const int normalSpiSpeedHz = 20000000L;

	//----- SPI速度設定 -----
	s_bitRate = normalSpiSpeedHz;
